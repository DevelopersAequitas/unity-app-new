    ...
    public function verifyOtp(string $emailOrPhone, string $otp): array
    {
        ...
        return [
            ...
            'user' => [
                ...
                'company_name' => (string) ($user->company_name ?? $user->business_name ?? 'Apex Dynamics'),
                'company' => (string) ($user->company_name ?? $user->business_name ?? 'Apex Dynamics'),
                'city' => (string) ($user->city ?? $user->city_of_residence ?? 'Ahmedabad'),
                'location' => (string) ($user->city ?? $user->city_of_residence ?? 'Ahmedabad'),
                'designation' => (string) ($user->designation ?? $user->job_title ?? 'Founder & CEO'),
                'business_category' => (string) ($user->industry ?? $user->businessCategory?->name ?? 'Technology'),
                'industry' => (string) ($user->industry ?? $user->businessCategory?->name ?? 'Technology'),
                'level_4_category' => (string) ($user->level4Category?->name ?? $user->business_sub_category ?? 'FinTech SaaS'),
                'level4_category' => (string) ($user->level4Category?->name ?? $user->business_sub_category ?? 'FinTech SaaS'),
                'profile_photo_url' => $avatarUrl,
                'avatar_url' => $avatarUrl,
                'life_impact' => (int) ($user->life_impacted_count ?? $user->impact_count ?? 0),
                'life_impacted_count' => (int) ($user->life_impacted_count ?? $user->impact_count ?? 0),
                'role' => $role,
