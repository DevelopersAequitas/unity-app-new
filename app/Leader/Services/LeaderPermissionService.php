...
use App\Leader\Models\LeaderRoleCapability;
...
class LeaderPermissionService
{
    public function __construct(
        private readonly UserRoleResolverService $roleResolver,
    ) {}
    ...
    /**
     * Map database role key/name to canonical standard role key (Delegated).
     */
    public function normalizeRoleKey(string $rawRole): string
    {
        return $this->roleResolver->normalizeRoleKey($rawRole);
    }

    /**
     * Determine if a given role key is an authorized leadership role (Delegated).
     */
    public function isLeaderRole(string $rawRole): bool
    {
        return $this->roleResolver->isLeaderRole($rawRole);
    }

    /**
     * Check if a given User is an authorized Leader (Delegated).
     */
    public function isLeader(User $user): bool
    {
        return $this->roleResolver->isLeader($user);
    }

    /**
     * Canonical display label for a role (Delegated).
     */
    public function getRoleLabel(string $canonicalRole): string
    {
        return $this->roleResolver->getRoleLabel($canonicalRole);
    }

    /**
     * Detect the user's primary leader role and custom label (Delegated).
     *
    ...
    {
        return $this->roleResolver->resolveUserRole($user);
    }

    /**
     * Resolve regional scope string (Delegated).
     */
    public function resolveRegionalScope(string $role): string
    {
        return $this->roleResolver->resolveRegionalScope($role);
    }
