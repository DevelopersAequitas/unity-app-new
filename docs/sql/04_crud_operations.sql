-- ============================================================
-- DYNAMIC RBAC SYSTEM — CRUD OPERATIONS
-- Unity App - PeersUnity
-- Reference for INSERT, UPDATE, DELETE operations
-- Replace <UUID> placeholders with actual UUID values
-- ============================================================
...
VALUES (UUID(), 'regional_manager', 'Regional Manager', 'Manages regional circles', 'admin', 'optional', 'active', 1, 'regional_manager', 0, NOW(), NOW());
...
VALUES (UUID(), '<PARENT_ROLE_UUID>', '<CHILD_ROLE_UUID>', NOW(), NOW());
...
VALUES (UUID(), 'Finance & Analytics', 'finance-analytics', 'bi-cash-stack', 17, 1, NOW(), NOW());
...
VALUES (UUID(), '<MODULE_UUID>', 'Member Audit Logs', 'admin.users.audit-logs', 'member-audit-logs', 'bi-shield-check', 11, 1, NULL, NOW(), NOW());
...
VALUES (UUID(), '<ROLE_UUID>', '<PAGE_UUID>', (SELECT id FROM permissions WHERE `key` = 'view'), NOW(), NOW());

-- Grant multiple permissions at once
INSERT INTO role_page_permissions (id, role_id, page_id, permission_id, created_at, updated_at)
VALUES
    (UUID(), '<ROLE_UUID>', '<PAGE_UUID>', (SELECT id FROM permissions WHERE `key` = 'view'),   NOW(), NOW()),
    (UUID(), '<ROLE_UUID>', '<PAGE_UUID>', (SELECT id FROM permissions WHERE `key` = 'create'), NOW(), NOW()),
    (UUID(), '<ROLE_UUID>', '<PAGE_UUID>', (SELECT id FROM permissions WHERE `key` = 'edit'),   NOW(), NOW()),
    (UUID(), '<ROLE_UUID>', '<PAGE_UUID>', (SELECT id FROM permissions WHERE `key` = 'export'), NOW(), NOW());
...
INSERT INTO role_module_access (id, role_id, module_id, is_visible, created_at, updated_at)
SELECT UUID(), '<TARGET_ROLE_UUID>', module_id, is_visible, NOW(), NOW()
...
INSERT INTO role_data_scope (id, role_id, admin_user_id, scope_type, scope_id, created_at, updated_at)
VALUES (UUID(), NULL, '<ADMIN_USER_UUID>', 'circle', '<CIRCLE_UUID>', NOW(), NOW());

-- Create global scope
INSERT INTO role_data_scope (id, role_id, admin_user_id, scope_type, scope_id, created_at, updated_at)
VALUES (UUID(), '<ROLE_UUID>', NULL, 'global', NULL, NOW(), NOW());
...
VALUES (UUID(), '<ADMIN_USER_UUID>', '<ROLE_UUID>', NULL, 'edit', NOW(), NOW());

-- Assign with allowed sections and view-only
INSERT INTO admin_user_roles (id, user_id, role_id, allowed_sections, permission_type, created_at, updated_at)
VALUES (UUID(), '<ADMIN_USER_UUID>', '<ROLE_UUID>', '["Dashboard", "Activities", "Members"]', 'view', NOW(), NOW());
...
VALUES (UUID(), '<MODULE_UUID>', 'Circle Join Request Approval', '<ROLE_UUID>', 1, 1, 'DED must approve first', NOW(), NOW());

-- Add second step to workflow
INSERT INTO workflow_approval_rules (id, module_id, workflow_name, approver_role_id, step_order, is_active, description, created_at, updated_at)
VALUES (UUID(), '<MODULE_UUID>', 'Circle Join Request Approval', '<ROLE_UUID_2>', 2, 1, 'Global Admin final approval', NOW(), NOW());
...
VALUES (UUID(), '<ADMIN_USER_UUID>', '<INDUSTRY_UUID>', 'IT & Software', '<ASSIGNER_UUID>', 1, NOW(), NOW());
