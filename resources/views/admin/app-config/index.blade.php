...
@endif
@if($active==='labels')
<div class="card ac-card labels-card">
    <div class="card-header d-flex flex-wrap justify-content-between gap-2 align-items-start">
        <div>
            <h2 class="h5 mb-1">Labels</h2>
            <p class="text-muted small mb-0">Edit text shown in the app without changing code. <span class="badge bg-light text-dark border">{{ $labels->total() }} total</span></p>
        </div>
        <button class="btn btn-success" form="labelsBulkForm"><i class="bi bi-save me-1"></i>Save All Changed Labels</button>
    </div>

    <form method="GET" action="{{ route('admin.app-config.index') }}" class="labels-filter-form">
        <input type="hidden" name="tab" value="labels">
        <div class="labels-filter-row">
            <div class="labels-filter-search">
                <input type="text" class="labels-filter-control" name="label_search" value="{{ request('label_search') }}" placeholder="Search labels by key, value, group, or description">
            </div>
            <div class="labels-filter-group">
                <select class="labels-filter-control" name="label_group">
                    <option value="all">All groups</option>
                    @foreach($labelGroups as $group)
                        <option value="{{ $group }}" @selected(request('label_group', 'all') === $group)>{{ $group }}</option>
                    @endforeach
                </select>
            </div>
            <div class="labels-filter-status">
                <select class="labels-filter-control" name="label_status">
                    <option value="all">All statuses</option>
                    <option value="active" @selected(request('label_status', 'all') === 'active')>Active</option>
                    <option value="inactive" @selected(request('label_status', 'all') === 'inactive')>Inactive</option>
                </select>
            </div>
    ...
    <form id="labelsBulkForm" method="POST" action="{{ route('admin.app-config.labels') }}">
        @csrf @method('PUT')
        <div class="labels-list-table">
            <table id="labelsTable" class="labels-table">
                <thead>
                    <tr><th>Label Key</th><th>Label Value</th><th>Group Name</th><th>Description</th><th>Active</th><th class="text-end">Action</th></tr>
                </thead>
                <tbody>
                    @forelse($labels as $l)
                        <tr data-group="{{ $l->group_name }}" data-active="{{ $l->is_active ? '1':'0' }}">
                            <td><span class="mono-badge">{{ $l->label_key }}</span></td>
                            <td><input class="form-control form-control-sm js-dirty" name="labels[{{ $l->label_key }}][label_value]" value="{{ $l->label_value }}"></td>
                            <td><input class="form-control form-control-sm js-dirty" name="labels[{{ $l->label_key }}][group_name]" value="{{ $l->group_name }}"></td>
                            <td><input class="form-control form-control-sm js-dirty label-description-input" name="labels[{{ $l->label_key }}][description]" value="{{ $l->description }}"></td>
                            <td><div class="form-check form-switch"><input type="checkbox" class="form-check-input switch js-dirty" name="labels[{{ $l->label_key }}][is_active]" value="1" @checked($l->is_active)></div></td>
                            <td class="text-end"><button class="btn btn-sm btn-outline-success">Save</button></td>
                        </tr>
                    @empty
                        <tr><td colspan="6"><div class="empty-state">No labels found.</div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="labels-bulk-actions">
            <button class="btn btn-success">Bulk Save Labels</button>
            <button type="reset" class="btn btn-outline-secondary ms-2">Discard changes</button>
            <span class="small text-muted ms-2 unsaved-count"></span>
        </div>
    </form>

    <div class="labels-pagination-footer">
        <div class="small text-muted">Showing {{ $labels->firstItem() ?? 0 }} to {{ $labels->lastItem() ?? 0 }} of {{ $labels->total() }} results</div>
        <div>{{ $labels->withQueryString()->links() }}</div>
        <div class="small text-muted">Showing {{ $labels->firstItem() ?? 0 }}-{{ $labels->lastItem() ?? 0 }} of {{ $labels->total() }} records</div>
    </div>
</div>
@endif
...
@if($active==='icons')
    @include('admin.app-config.partials.icons', ['icons' => $icons])
@endif
