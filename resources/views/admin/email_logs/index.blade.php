    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2"><label class="form-label small text-muted">Status</label><select name="status" form="emailLogsFiltersForm" class="form-select"><option value="all" @selected($filters['status'] === 'all')>All</option><option value="queued" @selected($filters['status'] === 'queued')>Queued</option><option value="sent" @selected($filters['status'] === 'sent')>Sent</option><option value="failed" @selected($filters['status'] === 'failed')>Failed</option><option value="pending" @selected($filters['status'] === 'pending')>Pending</option></select></div>
                <div class="col-md-2"><label class="form-label small text-muted">Triggered By</label><select name="triggered_by" form="emailLogsFiltersForm" class="form-select"><option value="all" @selected($filters['triggered_by'] === 'all')>All</option><option value="admin" @selected($filters['triggered_by'] === 'admin')>Admin</option><option value="user" @selected($filters['triggered_by'] === 'user')>User</option><option value="system" @selected($filters['triggered_by'] === 'system')>System</option><option value="scheduled_job" @selected($filters['triggered_by'] === 'scheduled_job')>Scheduled Job</option><option value="queue_worker" @selected($filters['triggered_by'] === 'queue_worker')>Queue Worker</option></select></div>
                <div class="col-md-2"><label class="form-label small text-muted">Admin ID</label><input type="text" name="admin" form="emailLogsFiltersForm" value="{{ $filters['admin'] }}" class="form-control"></div>
                <div class="col-md-2"><label class="form-label small text-muted">User ID</label><input type="text" name="user" form="emailLogsFiltersForm" value="{{ $filters['user'] }}" class="form-control"></div>
                ...
                <div class="col-md-2"><label class="form-label small text-muted">Per Page</label><select name="per_page" form="emailLogsFiltersForm" class="form-select">@foreach ([10, 20, 50, 100] as $perPageOption)<option value="{{ $perPageOption }}" @selected($filters['per_page'] == $perPageOption)>{{ $perPageOption }}</option>@endforeach</select></div>
