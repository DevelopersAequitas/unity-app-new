...
REVERB_APP_SECRET=<existing-live-secret-or-secure-secret>
...
curl -i https://peersunity.com/app/z8hlmhiqxac2alirbdtx?protocol=7\&client=flutter\&version=1.0\&flash=false
...
## Live-server-only configuration locations

Do not add Nginx or Supervisor configuration to Laravel, PHP, Dart, or Flutter files. Apply the WebSocket proxy only in `/etc/nginx/sites-available/peersunity.com` inside the active HTTPS `server` block, and apply the process manager only in `/etc/supervisor/conf.d/peersunity-reverb.conf`.

## Flutter service note

This repository does not contain the real Flutter application or an existing `realtime_service.dart`. Do not replace the real Flutter `main.dart`. In the real Flutter realtime service only, ensure these values are used: app key `z8hlmhiqxac2alirbdtx`, host `peersunity.com`, `wssPort: 443`, `useTLS: true`, `enabledTransports: ['wss']`, and auth endpoint `https://peersunity.com/api/broadcasting/auth`.
