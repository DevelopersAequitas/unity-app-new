    ...
    <form method="POST" action="{{ $isEdit ? route('admin.events.update', $event->id) : route('admin.events.store') }}" id="eventCreateForm" enctype="multipart/form-data">
        ...
        <div class="card mb-3">
            ...
            <div class="card-body row g-3">
                ...
                <div class="col-12"><label class="form-label">Popup Message</label><textarea class="form-control" name="popup_message" rows="3">{{ old('popup_message', $isEdit ? ($event->popup_message ?? '') : 'A new event is available. Register now.') }}</textarea></div>
            </div>
        </div>
