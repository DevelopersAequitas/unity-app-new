...
<div class="card mb-3">
    ...
    <div class="card-body row g-3">
        ...
        <div class="col-md-3"><label class="form-label">City</label><select name="city_id" class="form-select js-searchable-select"><option value="">Select City</option>@foreach($cities as $city)<option value="{{ $city->id }}" @selected(old('city_id', $circular->city_id)===(string)$city->id)>{{ $city->name }}</option>@endforeach</select></div>
        <div class="col-md-3"><label class="form-label">Circle</label><select name="circle_id" class="form-select js-searchable-select"><option value="">Select Circle</option>@foreach($circles as $circle)<option value="{{ $circle->id }}" @selected(old('circle_id', $circular->circle_id)===(string)$circle->id)>{{ $circle->name }}</option>@endforeach</select></div>
