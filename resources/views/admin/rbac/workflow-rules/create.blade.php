...
@section('title', 'Add Workflow Rule — RBAC')
...
<div class="mb-4">
    ...
    <h1 class="h4 fw-bold mb-0">Add Workflow Approval Rule</h1>
</div>

<div class="card border-0 shadow-sm" style="max-width:600px">
    <div class="card-body">
        <form action="{{ route('admin.rbac.workflow-rules.store') }}" method="POST">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="workflow_action" class="form-control @error('workflow_action') is-invalid @enderror" value="{{ old('workflow_action') }}" placeholder="e.g. approve_membership">
            ...
            <div class="mb-3">
                ...
                <select name="approver_role_id" class="form-select @error('approver_role_id') is-invalid @enderror">
                    ...
                    @foreach($roles as $role)
                        <option value="{{ $role->id }}" {{ old('approver_role_id') == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i> Save Rule</button>
        </form>
    </div>
</div>
@endsection
