...
<div class="card p-0 overflow-auto">
 ...
 <table class="table table-sm mb-0"><thead><tr><th>ID</th><th>User</th><th>Source</th><th>Status</th><th>Amount</th></tr></thead><tbody>@forelse($latestPayments as $p)<tr><td>{{ $p->id }}</td><td>{{ $p->user_id }}</td><td>{{ $p->source }}</td><td>{{ $p->status }}</td><td>₹{{ number_format((float)$p->amount,2) }}</td></tr>@empty<tr><td colspan="5" class="text-center text-muted py-3">No payment records.</td></tr>@endforelse</tbody></table>
</div>
@endsection
