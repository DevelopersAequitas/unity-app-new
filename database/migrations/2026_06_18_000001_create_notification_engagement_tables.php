    ...
    public function up(): void
    {
        ...
        if (! Schema::hasTable('notification_campaign_runs')) {
            Schema::create('notification_campaign_runs', function (Blueprint $table): void {
                ...
                $table->integer('audience_count')->default(0); $table->integer('sent_count')->default(0); $table->integer('failed_count')->default(0); $table->integer('skipped_count')->default(0);
                $table->timestamp('started_at')->nullable(); $table->timestamp('finished_at')->nullable();
                $table->jsonb('meta')->default(DB::raw("'{}'::jsonb")); $table->timestamps();
                $table->foreign('campaign_id')->references('id')->on('notification_campaigns')->cascadeOnDelete();
                $table->index(['campaign_id', 'created_at']); $table->index('status');
            });
        }

        if (! Schema::hasTable('notification_delivery_logs')) {
            Schema::create('notification_delivery_logs', function (Blueprint $table): void {
                $table->uuid('id')->primary(); $table->uuid('notification_id'); $table->uuid('user_id');
                $table->string('channel'); $table->string('provider')->nullable(); $table->string('provider_message_id')->nullable(); $table->string('status')->default('pending');
                $table->jsonb('request_payload')->default(DB::raw("'{}'::jsonb")); $table->jsonb('response_payload')->default(DB::raw("'{}'::jsonb"));
                $table->text('error_message')->nullable(); $table->timestamp('attempted_at')->nullable(); $table->timestamp('delivered_at')->nullable(); $table->timestamps();
                $table->foreign('notification_id')->references('id')->on('app_notifications')->cascadeOnDelete(); $table->foreign('user_id')->references('id')->on('users')->cascadeOnDelete();
                $table->index(['notification_id', 'channel']); $table->index(['user_id', 'status', 'created_at']);
            });
        }

        if (! Schema::hasTable('notification_preferences')) {
            Schema::create('notification_preferences', function (Blueprint $table): void {
                $table->uuid('id')->primary(); $table->uuid('user_id')->unique();
                foreach (['push','email','chat','event','circle','business','campaign'] as $name) { $table->boolean($name.'_enabled')->default(true); }
                $table->time('quiet_hours_start')->nullable(); $table->time('quiet_hours_end')->nullable(); $table->jsonb('config')->default(DB::raw("'{}'::jsonb")); $table->timestamps();
                $table->foreign('user_id')->references('id')->on('users')->cascadeOnDelete();
            });
        }
        ...
        if (! Schema::hasTable('notification_suppression_logs')) {
            Schema::create('notification_suppression_logs', function (Blueprint $table): void {
                $table->uuid('id')->primary(); $table->uuid('user_id'); $table->uuid('campaign_id')->nullable(); $table->string('type'); $table->string('dedupe_key')->nullable();
                $table->timestamp('last_sent_at'); $table->integer('send_count')->default(1); $table->timestamps();
                $table->foreign('user_id')->references('id')->on('users')->cascadeOnDelete(); $table->foreign('campaign_id')->references('id')->on('notification_campaigns')->nullOnDelete();
                $table->index(['user_id', 'type', 'last_sent_at']); $table->index('campaign_id'); $table->index('dedupe_key'); $table->unique(['user_id', 'type', 'dedupe_key'], 'notification_suppression_user_type_dedupe_unique');
            });
        }
