...
<div class="container-fluid py-3">
    ...
    <form class="card card-body mb-3" method="GET">
        <div class="row g-2">
            ...
            <div class="col-md-2"><select class="form-select" name="event_type"><option value="">All Types</option>@foreach(['circle_meeting','city_event','global_event','public_event','training'] as $type)<option value="{{ $type }}" @selected(request('event_type')===$type)>{{ str($type)->replace('_', ' ')->title() }}</option>@endforeach</select></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                @forelse($events as $event)
                    <tr>
                        <td>{{ $event->title }}</td><td>{{ str($event->event_type)->replace('_', ' ')->title() }}</td><td>@if(in_array($event->event_type, ['city_event', 'global_event'], true) && $event->circles->isNotEmpty()){{ $event->circles->pluck('name')->join(', ') }}@else{{ $event->circle?->name ?? '-' }}@endif</td><td>{{ $event->mode }}</td>
