...
@section('title', 'Impact Guidelines')
...
<div id="grid-root-container" class="light rounded-xl border bs p-4 relative admin-grid-card space-y-6">
    ...
    <div class="flex flex-wrap justify-between items-center gap-3">
        <div>
            <h2 class="font-display font-semibold text-xs text-rose-500 uppercase tracking-wider m-0 flex items-center gap-1.5">
                <span>❤️</span> Impact Guidelines Management
            </h2>
            <p class="text-xs t3 m-0 mt-0.5">Manage actions, categories, life impact values, units, and ordering for the Flutter app's Impact Guidelines screen.</p>
        </div>
        <div class="flex items-center gap-2.5 flex-wrap">
            <button type="button" class="btn btn-sm btn-outline-primary text-xs font-semibold shadow-sm px-3 py-1.5 d-inline-flex align-items-center gap-1.5" data-bs-toggle="modal" data-bs-target="#editHeaderModal">
                <i class="bi bi-gear-fill"></i> Edit Header Information
            </button>
            <a href="{{ route('admin.impact-guidelines.create') }}" class="btn btn-sm btn-primary text-xs font-semibold text-white shadow-sm px-3 py-1.5 d-inline-flex align-items-center gap-1.5 text-decoration-none">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="p-3.5 rounded-xl border bs bg-white shadow-sm flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center text-lg font-bold">
                ❤️
            </div>
            <div>
        ...
        <div class="p-3.5 rounded-xl border bs bg-white shadow-sm flex items-center gap-3">
            ...
            <div>
                <div class="text-[11px] t3 font-medium uppercase tracking-wider">Categories</div>
                <div class="text-lg font-bold text-purple-600">{{ $stats['categories_count'] }}</div>
            </div>
        </div>
        <div class="p-3.5 rounded-xl border bs bg-rose-50/50 flex flex-col justify-center">
            <div class="text-[11px] font-semibold text-rose-800">Mobile API Preview</div>
            <div class="text-[11px] text-rose-700 truncate mt-0.5"><code>GET /api/v1/impact-guidelines</code></div>
        </div>
    </div>

    {{-- Header Preview Card --}}
    <div class="p-4 rounded-xl border bs bg-gradient-to-r from-rose-50/70 via-white to-rose-50/30 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-start gap-3">
            <div class="w-12 h-12 rounded-xl bg-rose-100 border border-rose-200 flex items-center justify-center text-2xl flex-shrink-0">
                @if(!empty($icon))
            ...
            <div>
                <div class="flex items-center gap-2">
                    <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-rose-200/60 text-rose-900">Current Header Config</span>
        ...
        <button type="button" class="btn btn-sm btn-outline-danger text-xs font-semibold px-3 py-1 rounded-lg flex-shrink-0" data-bs-toggle="modal" data-bs-target="#editHeaderModal">
    ...
    <div class="p-3 rounded-xl border bs bg-white shadow-sm">
        <form method="GET" action="{{ route('admin.impact-guidelines.index') }}" class="flex flex-wrap items-center gap-3">
            <div class="flex-grow min-w-[200px]">
                <div class="input-group input-group-sm">
                    <span class="input-group-text bg-light text-muted"><i class="bi bi-search"></i></span>
                    <input type="text" name="q" class="form-control text-xs" placeholder="Search by action or category..." value="{{ $filters['search'] ?? '' }}">
                </div>
            </div>
            <div class="w-48">
                <select name="category" class="form-select form-select-sm text-xs">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.impact-guidelines.index') }}" class="btn btn-sm btn-outline-secondary text-xs px-3">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full border-collapse text-xs">
                <thead>
                    <tr class="text-[11px] uppercase tracking-wider text-gray-500 font-semibold bg-gray-50 border-b bs">
                        ...
                        <th class="py-3 px-4 text-left">Action Description</th>
                        <th class="py-3 px-4 text-center" style="width: 170px;">Category</th>
                        <th class="py-3 px-4 text-center" style="width: 140px;">Impact</th>
                        <th class="py-3 px-4 text-center" style="width: 120px;">Status</th>
                        <th class="py-3 px-4 text-right" style="width: 160px;">Actions</th>
                    </tr>
                </thead>
                <tbody id="impactGuidelinesTableBody" class="divide-y divide-gray-100">
                    @forelse ($guidelines as $guideline)
                        <tr class="hover:bg-rose-50/30 transition-colors" data-id="{{ $guideline->id }}">
                            ...
                            <td class="py-3 px-4">
                                <div class="font-semibold text-gray-900 text-sm">{{ $guideline->action }}</div>
                                <div class="text-[11px] text-gray-400 font-mono">ID: {{ $guideline->id }}</div>
                            </td>
                            <td class="py-3 px-4 text-center">
                                <span class="badge bg-purple-50 text-purple-700 border border-purple-200 px-2.5 py-1 rounded font-medium">
                                    {{ $guideline->category }}
                                </span>
                            </td>
                            <td class="py-3 px-4 text-center">
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-300">
                                    +{{ $guideline->impact_value }} {{ $guideline->impact_unit }}
                                </span>
                            </td>
                            <td class="py-3 px-4 text-center">
                                <form method="POST" action="{{ route('admin.impact-guidelines.toggle-status', $guideline->id) }}" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td class="py-3 px-4 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.impact-guidelines.edit', $guideline->id) }}" class="btn btn-sm btn-light border text-primary p-1.5 rounded hover:bg-primary hover:text-white transition" title="Edit Guideline">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.impact-guidelines.destroy', $guideline->id) }}" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this impact guideline?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-gray-400">
                                ...
                                No impact guidelines found matching your criteria.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="editHeaderModal" tabindex="-1" aria-labelledby="editHeaderModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-xl">
            <form method="POST" action="{{ route('admin.impact-guidelines.update-header') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-header bg-light border-b px-4 py-3">
                    <h5 class="modal-title text-sm font-bold text-gray-800" id="editHeaderModalLabel">
                        ❤️ Edit Impact Guidelines Header
                    </h5>
                ...
                <div class="modal-body p-4 space-y-3">
                    <div>
                        ...
                        <span class="text-[11px] text-muted">Example: Your Life Impact Score</span>
                    </div>
