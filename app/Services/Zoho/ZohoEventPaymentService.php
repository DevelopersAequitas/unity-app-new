...
class ZohoEventPaymentService
{
    ...
    {
        $registration->loadMissing(['event', 'occurrence', 'user']);
        if (! empty($registration->payment_url)) {
            return $registration;
        }
        try {
    ...
    public function syncPaidPaymentFromWebhook(array $payload): ?EventRegistration
    {
        ...
        $registration = $this->resolve($payload);
        if (! $registration) {
            return null;
        }
        $raw = strtolower((string) (data_get($payload, 'payment.status') ?? data_get($payload, 'invoice.status') ?? data_get($payload, 'status') ?? ''));
        if (str_contains($raw, 'paid') || str_contains($raw, 'success')) {
            return $this->markRegistrationPaid($registration, $payload);
        }
    ...
    {
        foreach ([data_get($payload, 'registration_id'), data_get($payload, 'reference_id'), data_get($payload, 'invoice.reference_number')] as $id) {
            if ($id) {
        ...
        foreach (['zoho_invoice_id' => [data_get($payload, 'invoice.invoice_id'), data_get($payload, 'invoice_id')], 'zoho_payment_id' => [data_get($payload, 'payment.payment_id'), data_get($payload, 'payment_id')], 'zoho_hosted_page_id' => [data_get($payload, 'hostedpage.hostedpage_id'), data_get($payload, 'hostedpage_id')]] as $c => $vals) {
            foreach ($vals as $v) {
                if ($v) {
                    $r = EventRegistration::query()->where($c, $v)->first();
                    if ($r) {
                        return $r;
                    }
                }
    ...
    private function f(array $data): array
    {
