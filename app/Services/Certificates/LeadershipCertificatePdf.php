...
class LeadershipCertificatePdf
{
    private const PAGE_WIDTH = 842;
    private const PAGE_HEIGHT = 595;

    public function generate(LeadershipCertificationSubmission $submission): string
    {
    ...
    public function filename(LeadershipCertificationSubmission $submission): string
    {
        return 'leadership-certificate-'.$submission->id.'.pdf';
    }
    ...
    private function buildContent(array $details): string
    {
        ...
        $commands[] = $this->centerText('Leadership Certification', 434, 16, 'F2', [0.46, 0.25, 0.70]);
        ...
        $commands[] = $this->centerText('for successfully achieving the Leadership Certification with the level', 318, 12, 'F1', [0.36, 0.36, 0.44]);
        $commands[] = $this->centerText($details['certification_level'], 290, 18, 'F2', [0.30, 0.13, 0.55]);
        ...
        $rows = [
            ...
            ['Contact Number', $details['contact_no'], 'Certification Level', $details['certification_level']],
