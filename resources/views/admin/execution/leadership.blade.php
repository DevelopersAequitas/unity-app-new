...
<div class="row g-3 mb-3">
  ...
  <div class="col-12"><div class="card p-0 overflow-auto"><table class="table table-sm mb-0"><thead><tr><th>Name</th><th>Email</th><th>Status</th><th>Date</th></tr></thead><tbody>@forelse($applications as $app)<tr><td>{{ $app->name ?? $app->full_name ?? '-' }}</td><td>{{ $app->email ?? '-' }}</td><td><span class="badge bg-secondary">{{ $app->status ?? 'pending' }}</span></td><td>{{ optional($app->created_at)->format('d M Y') }}</td></tr>@empty<tr><td colspan="4" class="text-center text-muted py-3">No applications found.</td></tr>@endforelse</tbody></table></div>{{ $applications->withQueryString()->links() }}</div>
</div>
<div class="row g-3">
  <div class="col-lg-7"><div class="card p-0 overflow-auto"><div class="p-3 border-bottom"><h6 class="mb-0">Active Assignments</h6></div><table class="table table-sm mb-0"><thead><tr><th>User</th><th>Role</th><th>Circle</th><th>Status</th></tr></thead><tbody>@forelse($assignments as $row)<tr><td>{{ $row->display_name }}</td><td><span class="badge bg-info text-dark">{{ $row->role }}</span></td><td>{{ $row->circle_name }}</td><td>{{ $row->status }}</td></tr>@empty<tr><td colspan="4" class="text-center text-muted py-3">No assignments found.</td></tr>@endforelse</tbody></table></div>{{ $assignments->withQueryString()->links() }}</div>
