...
@section('title', 'All Available Notifications Lists')
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold text-dark">All Available Notifications Lists</h4>
            <p class="text-muted small mb-0">Browse, search, and manage system push notifications in real-time.</p>
        </div>
        <div>
            <span class="badge bg-primary px-3 py-2 fs-6 rounded-pill" style="background-color: #240e5c !important;">
                {{ count($templates) }} Notifications Registered
            </span>
        </div>
    </div>
            ...
            <div class="row g-3 align-items-center">
                <div class="col-md-6 col-lg-4">
                    <div class="input-group">
                        ...
                        <input type="text" id="notificationSearch" class="form-control bg-light border-0" placeholder="Search notifications by name or key..." onkeyup="filterNotifications()">
                    </div>
                </div>
                <div class="col-md-6 col-lg-8 text-md-end text-muted small">
                    Showing <span id="visibleCount" class="fw-bold text-dark">{{ count($templates) }}</span> of {{ count($templates) }} notifications
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Cards Grid -->
    <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4" id="notificationsGrid">
        @foreach($templates as $tpl)
            <div class="col notification-card-item" data-name="{{ strtolower($tpl['name']) }}" data-key="{{ strtolower($tpl['key']) }}">
...
<script>
    function filterNotifications() {
        ...
        const cards = document.getElementsByClassName('notification-card-item');
        let visibleCount = 0;
