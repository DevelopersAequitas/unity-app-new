...
{{ $payments->links() }}
<div class="card p-0 overflow-auto"><div class="p-3 border-bottom"><h6 class="mb-0">Active Subscriptions (Zoho)</h6></div><table class="table table-sm mb-0"><thead><tr><th>User</th><th>Subscription</th><th>Plan</th><th>Status</th></tr></thead><tbody>@forelse($subscriptions as $s)<tr><td>{{ $s->display_user }}</td><td>{{ $s->zoho_subscription_id }}</td><td>{{ $s->zoho_plan_code }}</td><td>{{ \App\Support\Membership\MembershipStatusLabels::label($s->membership_status) }}</td></tr>@empty<tr><td colspan="4" class="text-center text-muted py-3">No subscriptions found.</td></tr>@endforelse</tbody></table></div>
@endsection
