<?php

declare(strict_types=1);

namespace Tests\Feature;

use App\Models\AdminUser;
use App\Models\CoinGuideline;
use App\Models\Role;
use App\Models\User;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Str;
use Tests\TestCase;

class CoinGuidelinesAdminTest extends TestCase
{
    protected AdminUser $admin;

    protected function setUp(): void
    {
        parent::setUp();

        Schema::dropIfExists('users');
        Schema::create('users', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->string('first_name', 100)->nullable();
            $table->string('last_name', 100)->nullable();
            $table->string('display_name', 150)->nullable();
            $table->string('email', 255)->unique();
            $table->string('phone', 20)->nullable()->unique();
            $table->string('password_hash')->nullable();
            $table->string('status')->default('active');
            $table->timestamps();
            $table->softDeletes();
        });

        Schema::dropIfExists('circle_members');
        Schema::create('circle_members', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->uuid('user_id')->nullable();
            $table->uuid('circle_id')->nullable();
            $table->string('status')->default('approved');
            $table->timestamps();
            $table->softDeletes();
        });

        Schema::dropIfExists('admin_users');
        Schema::create('admin_users', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->string('name');
            $table->string('email')->unique();
            $table->timestamps();
        });

        Schema::dropIfExists('roles');
        Schema::create('roles', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->string('key')->unique();
            $table->string('name');
            $table->timestamps();
        });

        Schema::dropIfExists('admin_user_roles');
        Schema::create('admin_user_roles', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->uuid('user_id');
            $table->uuid('role_id');
            $table->timestamps();
        });

        Schema::dropIfExists('coin_guidelines');
        Schema::create('coin_guidelines', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->string('title')->default('The Coin Reward System');
            $table->text('description')->nullable();
            $table->string('icon')->nullable();
            $table->string('activity');
            $table->unsignedInteger('coins')->default(0);
            $table->integer('display_order')->default(1);
            $table->boolean('is_active')->default(true);
            $table->timestamps();
            $table->softDeletes();
            $table->index(['is_active', 'display_order']);
        });

        User::query()->create([
            'id' => (string) Str::uuid(),
            'email' => 'superadmin@peersunity.com',
            'display_name' => 'Super Admin',
            'status' => 'active',
        ]);

        $this->admin = AdminUser::query()->create([
            'id' => (string) Str::uuid(),
            'name' => 'Super Admin',
            'email' => 'superadmin@peersunity.com',
        ]);

        $superRole = Role::query()->create([
            'id' => (string) Str::uuid(),
            'key' => 'super_admin',
            'name' => 'Super Admin',
        ]);

        DB::table('admin_user_roles')->insert([
            'id' => (string) Str::uuid(),
            'user_id' => $this->admin->id,
            'role_id' => $superRole->id,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }

    public function test_admin_can_view_coin_guidelines_index(): void
    {
        CoinGuideline::query()->create([
            'activity' => 'Test Activity',
            'coins' => 5000,
            'display_order' => 1,
            'is_active' => true,
        ]);

        $response = $this->actingAs($this->admin, 'admin')
            ->get(route('admin.coin-guidelines.index'));

        $response->assertStatus(200);
        $response->assertSee('Test Activity');
        $response->assertSee('5,000');
    }

    public function test_admin_can_create_coin_guideline(): void
    {
        $response = $this->actingAs($this->admin, 'admin')
            ->post(route('admin.coin-guidelines.store'), [
                'activity' => 'New Referral Deal',
                'coins' => 7500,
                'display_order' => 8,
                'is_active' => '1',
            ]);

        $response->assertRedirect(route('admin.coin-guidelines.index'));
        $this->assertDatabaseHas('coin_guidelines', [
            'activity' => 'New Referral Deal',
            'coins' => 7500,
            'display_order' => 8,
            'is_active' => true,
        ]);
    }

    public function test_admin_can_update_coin_guideline(): void
    {
        $guideline = CoinGuideline::query()->create([
            'activity' => 'Old Activity',
            'coins' => 1000,
            'display_order' => 1,
            'is_active' => true,
        ]);

        $response = $this->actingAs($this->admin, 'admin')
            ->put(route('admin.coin-guidelines.update', $guideline->id), [
                'activity' => 'Updated Activity',
                'coins' => 2500,
                'display_order' => 3,
                'is_active' => '1',
            ]);

        $response->assertRedirect(route('admin.coin-guidelines.index'));
        $this->assertDatabaseHas('coin_guidelines', [
            'id' => $guideline->id,
            'activity' => 'Updated Activity',
            'coins' => 2500,
            'display_order' => 3,
        ]);
    }

    public function test_admin_can_toggle_guideline_status(): void
    {
        $guideline = CoinGuideline::query()->create([
            'activity' => 'Toggle Activity',
            'coins' => 1000,
            'display_order' => 1,
            'is_active' => true,
        ]);

        $response = $this->actingAs($this->admin, 'admin')
            ->patch(route('admin.coin-guidelines.toggle-status', $guideline->id));

        $response->assertRedirect();
        $this->assertFalse($guideline->fresh()->is_active);

        // Toggle back
        $this->actingAs($this->admin, 'admin')
            ->patch(route('admin.coin-guidelines.toggle-status', $guideline->id));

        $this->assertTrue($guideline->fresh()->is_active);
    }

    public function test_admin_can_delete_guideline(): void
    {
        $guideline = CoinGuideline::query()->create([
            'activity' => 'To Delete',
            'coins' => 500,
            'display_order' => 1,
            'is_active' => true,
        ]);

        $response = $this->actingAs($this->admin, 'admin')
            ->delete(route('admin.coin-guidelines.destroy', $guideline->id));

        $response->assertRedirect(route('admin.coin-guidelines.index'));
        $this->assertSoftDeleted('coin_guidelines', ['id' => $guideline->id]);
    }

    public function test_admin_can_update_header_info(): void
    {
        CoinGuideline::query()->create([
            'activity' => 'Item 1',
            'coins' => 1000,
            'display_order' => 1,
        ]);

        $response = $this->actingAs($this->admin, 'admin')
            ->post(route('admin.coin-guidelines.update-header'), [
                'title' => 'Updated Coin Rewards',
                'description' => 'Brand new coin rewards description.',
                'icon_url' => 'https://example.com/coin.png',
            ]);

        $response->assertRedirect();
        $this->assertDatabaseHas('coin_guidelines', [
            'title' => 'Updated Coin Rewards',
            'description' => 'Brand new coin rewards description.',
            'icon' => 'https://example.com/coin.png',
        ]);
    }
}
