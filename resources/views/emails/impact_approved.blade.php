            ...
            <table style="background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05);" width="600" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <td style="padding: 14px 14px; background-color: #240e5c; text-align: center;">
                        <img style="vertical-align: middle;" src="https://peersunity.com/images/peersglobal-logo.png" alt="Peers Global" width="135" />
                    </td>
                </tr>
                <tr>
                    <td style="padding: 14px 16px; font-size: 16px; color: #333333;">
                        Dear <strong>{{ $submitter->display_name ?? trim(($submitter->first_name ?? '') . ' ' . ($submitter->last_name ?? '')) ?: 'Peer' }}</strong>,<br /><br />
                        Great news! Your Impact has been approved successfully.<br /><br />
                        ...
                        <strong>Status:</strong> {{ ucfirst($impact->status) }}<br />
                        <strong>Life Impacted:</strong> {{ (int) ($impact->life_impacted ?? 1) }}<br />
                        <strong>Total Life Impacted:</strong> {{ (int) ($submitter->life_impacted_count ?? 0) }}<br /><br />
                        Thank you for creating meaningful impact.<br /><br />
