...
<div class="card p-0 overflow-auto mb-3"><table class="table table-sm mb-0"><thead><tr><th>ID</th><th>User</th>@if($summary['supports_source_split'])<th>{{ ucfirst(str_replace('_', ' ', $summary['source_column'])) }}</th>@endif<th>Status</th><th>{{ ucfirst(str_replace('_', ' ', $summary['amount_column'])) }}</th><th>Date</th></tr></thead><tbody>@forelse($payments as $p)<tr><td>{{ $p->id }}</td><td>{{ $p->user_id }}</td>@if($summary['supports_source_split'])<td>{{ $p->display_source }}</td>@endif<td>{{ $p->status }}</td><td>₹{{ number_format((float)$p->display_amount,2) }}</td><td>{{ optional($p->created_at)->format('d M Y') }}</td></tr>@empty<tr><td colspan="{{ $summary['supports_source_split'] ? 6 : 5 }}" class="text-center text-muted py-3">No payments.</td></tr>@endforelse</tbody></table></div>
{{ $payments->links() }}
