        ...
        <section class="card">
            @if($isPendingPayment)
                <div class="alert alert-error">
                    <strong>Registration Pending — Payment Required</strong>
                    <div>Please complete payment to finalize your registration and receive your entry QR pass.</div>
                    <div class="muted" style="margin-top: 6px;">Registration ID: {{ $registration->id }}</div>
                    <div class="muted">Payment status: <span style="text-transform: capitalize; font-weight: 600; color: var(--danger);">{{ $payment['payment_status'] ?? $registration->payment_status ?? 'pending' }}</span></div>
                    @if(isset($payment['amount']) || isset($payment['currency']) || $event->ticket_price)
                        <div class="muted">Amount Payable: {{ $payment['amount'] ?? $registration->amount ?? $event->ticket_price }} {{ strtoupper($payment['currency'] ?? $registration->currency ?? data_get($event->metadata, 'currency', 'INR')) }}</div>
                    @endif
                    @if(!empty($payment['error']))
                        <div style="margin-top: 6px;">{{ $payment['error'] }}</div>
                    @endif
                    @if($paymentUrl)
                        <div style="margin-top: 14px;">
                            <a class="button payment-button" href="{{ $paymentUrl }}" target="_blank" rel="noopener">Pay Fees Now</a>
                        </div>
                    @else
                        <div class="muted" style="margin-top: 10px;">Payment link is being processed. Please refresh this page shortly.</div>
                    @endif
                </div>
            @else
