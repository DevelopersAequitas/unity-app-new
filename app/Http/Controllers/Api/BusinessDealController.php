    ...
    public function userBusinessDealsList(Request $request, string $userId): JsonResponse
    {
        ...
        $items = collect($paginator->items())->map(function (BusinessDeal $deal): array {
            ...
            return [
                ...
                'created_at' => $deal->created_at ? Carbon::parse($deal->created_at)->timezone('UTC')->format('Y-m-d H:i:s') : '',
                'updated_at' => $deal->updated_at ? Carbon::parse($deal->updated_at)->timezone('UTC')->format('Y-m-d H:i:s') : '',
                'from_user' => $fromUser ? [
