...
@if($active==='overview')
    ...
    <div class="row g-3 mb-4"><div class="col-lg-8"><div class="card ac-card h-100"><div class="card-body"><h2 class="h5 mb-1">Quick Access</h2><p class="text-muted small mb-3">Jump directly to the area you want to explain or update.</p><div class="row g-3">@foreach(['branding'=>'Edit Branding','labels'=>'Manage Labels','features'=>'Manage Features','navigation'=>'Manage Navigation','widgets'=>'Manage Widgets','social'=>'Manage Social Links','membership'=>'Manage Membership Labels','api-docs'=>'View API Docs'] as $key=>$label)<div class="col-md-6"><a class="quick-card" href="?tab={{ $key }}"><i class="bi {{ $tabs[$key]['icon'] }} me-2 text-success"></i>{{ $label }}</a></div>@endforeach</div></div></div></div><div class="col-lg-4"><div class="card ac-card mb-3"><div class="card-body"><h2 class="h6">Public API</h2><div class="d-flex align-items-center gap-2 mb-3"><span class="badge bg-primary">GET</span><code class="small">{{ $publicApi }}</code></div><button class="btn btn-outline-secondary btn-sm copy-btn" data-copy="{{ $publicApi }}"><i class="bi bi-clipboard me-1"></i>Copy URL</button><a href="{{ $publicApi }}" target="_blank" class="btn btn-success btn-sm">Open/Test API</a></div></div><div class="card ac-card"><div class="card-body"><h2 class="h6">Config Health</h2>@foreach([['Branding configured',filled($branding['app_name'] ?? null)],['Labels available',$labels->count()>0],['Navigation menu ready',$navigation->count()>0],['Social links configured',$enabledSocial>0],['Membership labels configured',$membershipLabels->count()>0]] as $h)<div class="d-flex justify-content-between py-1"><span>{{ $h[0] }}</span><span class="badge {{ $h[1]?'bg-success':'bg-danger' }}">{{ $h[1]?'Ready':'Needs review' }}</span></div>@endforeach</div></div></div></div>
@endif
...
<div class="accordion" id="navGroups">@foreach($menuTypes as $menu=>$label)@php($items=$navigation->where('menu_type',$menu))<div class="accordion-item mb-3"><h2 class="accordion-header"><button class="accordion-button {{ $loop->first?'':'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#nav-{{ $menu }}"><i class="bi bi-list-nested text-success me-2"></i>{{ $label }} <span class="badge bg-light text-dark border ms-2">{{ $items->count() }} items</span><span class="small text-muted ms-3">{{ $menuHelp[$menu] }}</span></button></h2><div id="nav-{{ $menu }}" class="accordion-collapse collapse {{ $loop->first?'show':'' }}" data-bs-parent="#navGroups"><div class="accordion-body p-0">@if($items->count())<div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Sort</th><th>Item Key</th><th>Display Label</th><th>Icon</th><th>Route</th><th>Feature</th><th>Enabled</th><th>Actions</th></tr></thead><tbody>@foreach($items as $n)<tr><form method="POST" action="{{ route('admin.app-config.navigation.update',$n->id) }}">@csrf @method('PUT')<input type="hidden" name="menu_type" value="{{ $menu }}"><td><input name="sort_order" value="{{ $n->sort_order }}" class="form-control form-control-sm js-dirty" type="number"></td><td><input name="item_key" value="{{ $n->item_key }}" class="form-control form-control-sm js-dirty"><input type="hidden" name="label_key" value="{{ $n->label_key }}"></td><td><input name="display_label" value="{{ $n->display_label }}" class="form-control form-control-sm js-dirty"></td><td><input name="icon" value="{{ $n->icon }}" class="form-control form-control-sm js-dirty"></td><td><input name="route_name" value="{{ $n->route_name }}" class="form-control form-control-sm js-dirty"></td><td><input name="feature_key" value="{{ $n->feature_key }}" class="form-control form-control-sm js-dirty"></td><td><div class="form-check form-switch"><input type="checkbox" class="form-check-input switch js-dirty" name="is_enabled" value="1" @checked($n->is_enabled)></div></td><td class="text-nowrap"><button class="btn btn-sm btn-outline-success">Save</button></form><form method="POST" class="d-inline" action="{{ route('admin.app-config.navigation.destroy',$n->id) }}" onsubmit="return confirm('Delete this navigation item?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Delete</button></form></td></tr>@endforeach</tbody></table></div>@else<div class="empty-state m-3">No {{ strtolower($label) }} items yet.</div>@endif</div></div></div>@endforeach</div>
@endif
@if($active==='widgets')
