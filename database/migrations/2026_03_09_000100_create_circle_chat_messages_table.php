    ...
    public function up(): void
    {
        if (! Schema::hasTable('circle_chat_messages')) {
            ...
            if (DB::connection()->getDriverName() !== 'sqlite') {
                DB::statement("ALTER TABLE circle_chat_messages ADD CONSTRAINT chk_circle_chat_message_type CHECK (message_type IN ('text','image','video'))");
                DB::statement("ALTER TABLE circle_chat_messages ADD CONSTRAINT chk_circle_chat_message_content CHECK ((message_type = 'text' AND message_text IS NOT NULL) OR (message_type IN ('image','video') AND file_path IS NOT NULL))");
            }
        }
    }
